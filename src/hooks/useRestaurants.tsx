...
export const useRestaurants = () => {
  ...
  const fetchRestaurants = async () => {
    ...
    try {
      ...
      const transformedData =
        data?.map((restaurant) => ({
          ...restaurant,
          creator: Array.isArray(restaurant.creator)
            ? restaurant.creator[0]
            : restaurant.creator,
        })) || [];
  ...
  const createRestaurant = async (
    restaurant: Omit<RestaurantInsert, "creator_id">
  ) => {
    if (!user || !profile)
      return {
        data: null,
        error: new Error("User not authenticated or profile not found"),
      };
    ...
    try {
      ...
      if (profileError || !profileCheck) {
        throw new Error(
          "Profile not found in database. Please try refreshing the page."
        );
      }

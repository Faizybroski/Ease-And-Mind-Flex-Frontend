...
const UserBillingDetailsPage = ({ userId, onBack }) => {
  ...
  // Get current month range from a date (YYYY-MM-DD)
  ...
  // Get NEXT month range (bug-proof, no timezone)
  function getNextMonthRange(dateStr) {
    const [year, month] = dateStr.split("-").map(Number);
    ...
    const nextMonthStart = `${nextYear}-${String(nextMonth).padStart(
      2,
      "0"
    )}-01`;
    const lastDayNext = new Date(nextYear, nextMonth, 0).getDate();
    const nextMonthEnd = `${nextYear}-${String(nextMonth).padStart(
      2,
      "0"
    )}-${lastDayNext}`;
  ...
  //                       console.log("THIS MONTH RANGE:", monthStart, monthEnd);

  //                       // This goes to your existing API
  //                       const { data, error } = await supabase.functions.invoke(
  //                         "create-checkout-session",
  //                         {
  //                           body: {
  ...
  //                     }}
  //                   >
  //                     💳 Create Stripe Link (This Month)
  //                   </button>

  //                   {/* ============================ */}
  //                   {/* 🟦 PREVIEW NEXT MONTH PRICE  */}
  //                   {/* ============================ */}

  //                   <button
  //                     className="px-2 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 ease-in-out fs-6 mt-3"
  //                     onClick={async () => {
  //                       // --------------------------
  //                       // NEXT MONTH CALCULATION
  //                       // --------------------------
  //                       const { nextMonthStart, nextMonthEnd } =
  //                         getNextMonthRange(booking.start_date);
  //                       console.log(
  //                         "NEXT MONTH RANGE:",
  //                         nextMonthStart,
  //                         nextMonthEnd
  ...
  //                       // FETCH OCCURRENCES
  //                       // --------------------------
  //                       const { data: occurrences, error: occErr } =
  //                         await supabase
  ...
  //                         toast({
  //                           title: "Error",
  //                           description: "Could not load recurring dates",
  //                           variant: "destructive",
  //                         });
  //                         return;
  //                       }

  //                       console.log("Occurrences (next month):", occurrences);

  //                       if (!occurrences || occurrences.length === 0) {
  //                         toast({
  //                           title: "No Bookings",
  //                           description: "No bookings found for next month",
  //                         });
  //                         return;
  //                       }
  ...
  //                         nextMonthTotal += slotPrices[occ.timeslot] ?? 0;
  //                       }

  //                       console.log("Next month price:", nextMonthTotal);

  //                       toast({
  //                         title: "Next Month Price",
  //                         description: `€ ${nextMonthTotal}`,
  //                       });
  //                     }}
  //                   >
  //                     🔍 Preview Next Month Price
    ...
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <header className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-primary/70 text-sm">{userProfile?.email}</p>
        </div>

        <div className="flex gap-2 w-full sm:w-auto">
          <Button
            className="text-primary bg-secondary hover:bg-primary border border-primary hover:text-secondary w-full sm:w-auto"
            variant="outline"
            onClick={onBack}
          >
            ← Rug
          ...
          <Button
            className="text-secondary bg-primary hover:bg-secondary border border-primary hover:text-primary w-full sm:w-auto"
            onClick={() => monthlyPayment(userProfile.id)}
          >
            Maandelijkse betaling
          </Button>
          ...
          {bookings.map((booking) => (
            ...
            >
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                {/* ---------- DATES ---------- */}
                <div className="flex-1 flex flex-col gap-1 text-sm text-primary/70">
                  {booking.date && (
                ...
                <div className="flex-1 flex flex-col gap-2">
                  <div className="flex items-center gap-1">
                    <span className="text-primary font-medium">Betaling:</span>
                    <p
                      className={
                        booking.payment_status === "Completed"
                          ? "text-green-600 font-medium"
              ...
              {booking.is_recurring && (
                <div className="flex flex-col gap-4 lg:grid lg:grid-cols-2 lg:gap-6">
                  ...
                  <div className="space-y-2">
                    {/* Desktop header */}
                    <div className="hidden sm:grid grid-cols-2 gap-4 text-sm font-semibold text-primary/70">
                      <span>Dag</span>
                      ...
                      {Object.entries(booking.day_time_slots).map(
                        ([day, slot]) => (
                          ...
                          >
                            ...
                            <span className="text-primary/80 sm:text-right">
                              {slot}
                            </span>
                          </div>
                        )
                  ...
                  <div className="flex flex-col gap-3">
                    {/* THIS MONTH */}
                    <button
                      className="w-full px-3 py-2 bg-green-600 text-white font-semibold rounded-xl shadow hover:shadow-lg hover:scale-[1.03] transition-all text-sm"
                      onClick={async () => {
                        const { monthStart, monthEnd } = getMonthRange(
                          booking.start_date
                        );
                    ...
                    {/* NEXT MONTH */}
                    <button
                      className="w-full px-3 py-2 bg-blue-600 text-white font-semibold rounded-xl shadow hover:shadow-lg hover:scale-[1.03] transition-all text-sm"
                      onClick={async () => {
                        const { nextMonthStart, nextMonthEnd } =
                          getNextMonthRange(booking.start_date);
                        ...
                        if (!occurrences?.length) {
                          toast({
                            ...
                            description:
                              "Geen boekingen gevonden voor volgende maand.",
                          });
                          return;
                        }
